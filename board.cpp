...
board::board():woc(0),boc(0){
    ...
    cells.at(OB)->setMarble('#'); // sets the obstacle 
}
...
bool board::validateMove(const char& m,const string& l,const int& n, const int& fd, const int& md, int& mtype, bool& scoreMove) const{
    if(!cell::validateLocation(l) || l==OB)// confirm existence of location, the leading marble can not be the obstacle
        return false;
    else{
        cell* cp = cells.at(l);//initialize to leading cell
        const char marble = cp->getMarble(); // the leading cells marble
        if(marble!=m)//trying to move a space or a marble that is not yours or the obstacle
            return false;
        for(int i=1;i<n;i++){
            cp=cp->getAdjacent(fd);
            if(cp==nullptr || marble!=cp->getMarble())//confirm availability and type of claimed resources this will also include the #
                return false;
        }
        if(fd==getAdjacentIndex(md)){//in-line move
            ...
            for(int i=0;i<n;i++){
                ...
                if(cp!=nullptr){//confirm availability of required resources
                    if(marble==cp->getMarble() || cp->getMarble() == '#')//met own marble instead of space, or met the obstacle 
                        return false;
            ...
            for(int i=0;i<n;i++){
                ...
                if(dp!=nullptr && '+' == dp->getMarble())//is there an empty space to move to? if there is a #, the space will not be empty 
                    cp=cp->getAdjacent(fd);
                else
                    return false;
            }
