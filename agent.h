...
struct movement{
    ...
    movement(const string&,const int&,const int&,const int&,const int&,const int&,const bool&, const int&); 
...
struct moveTNDPriority{//priority based on move types and distance to edge
   inline bool operator()(const movement& a, const movement& b) { // compares 2 movements to determine which one is better than the other, based on score
...
protected:
    char symbol;
    vector<vector<int> > pmd;//potential move details. Each row is a move
    vector<movement> findMoves(const board&, const int&) const;
public:
    agent(const char&);
    char getSymbol(){return symbol;}
    virtual movement move(movement const* const,board, const int&)=0;//pure virtual function, now accepts move number
};

class randAgent: public agent{
public:
    randAgent(const char&);
    movement move(movement const* const,board, const int&);
};
